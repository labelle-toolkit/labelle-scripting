...
//! example demonstrates. The crystal scripts run compiled inside the
//! game binary but reach the world exclusively through the Script
//! Runtime Contract; when a system wants engine-side dispatch (Zig-only
//! APIs, typed payload structs), it consumes the SAME events from the
//! same engine bus via the game-root `hooks/` convention: the assembler
//! scans `hooks/*.zig` automatically (no project.labelle field), folds
...
//!
//! So one `Labelle.emit("hunger__feed", …)` in crystal/spawner.cr reaches
...
//!   - the crystal `on_event` handler in crystal/hunger.cr on the NEXT
